...
export const mockChatHistory = { 'support': [{ from: 'support', text: 'Hello, how can we help you today?', time: '09:30 AM' }], 'APP-00124-team': [ { from: 'Dr. Abdullah', text: 'The first milestone report has been submitted.', time: '10:15 AM' }, { from: 'Fund Team', text: 'Thank you, it will be reviewed shortly.', time: '10:17 AM' }, ], 'general': [{ from: 'Fund Team', text: 'Reminder: Applications for the non-profit support program are now open.', time: 'Yesterday'}] };
export const mockDocumentTemplates = [ { id: 'milestone_report', name: 'Milestone Report', description: 'Used to document progress for a specific grant milestone.'}, { id: 'budget_request', name: 'Budget Modification Request', description: 'Used to request changes to the approved budget line items.' }, { id: 'final_report', name: 'Final Project Report', description: 'Used to provide a comprehensive summary of project outcomes and achievements.'} ];
export const evaluationTemplate = [ { id: 'c1', title: 'Environmental Impact', weight: 30, subCriteria: [ { id: 's1a', text: 'Contribution to carbon emission reduction' }, { id: 's1b', text: 'Contribution to biodiversity' } ] }, { id: 'c2', title: 'Feasibility & Sustainability', weight: 40, subCriteria: [ { id: 's2a', text: 'Realistic timeline and budget' }, { id: 's2b', text: 'Project sustainability plan after funding ends' }, { id: 's2c', text: 'Team experience' } ] }, { id: 'c3', title: 'Innovation & Creativity', weight: 30, subCriteria: [ { id: 's3a', text: 'Use of new technologies or innovative business models' } ] } ];
export const mockReviewAssignments = [ mockApplications.find(a => a.id === 'APP-00126'), mockApplications.find(a => a.id === 'APP-00128'), ];
export const mockIntegrations = [ { id: 'int1', name: 'ELM" for Identity Verification', status: 'active', icon: <HardDrive/>, description: 'To link and automatically verify applicant data.' }, { id: 'int2', name: 'Etimad" Platform for Tenders', status: 'inactive', icon: <HardDrive/>, description: 'To facilitate tendering for funded projects.' }, { id: 'int3', name: 'Nafath" for Identity Verification', status: 'active', icon: <ShieldCheck/>, description: 'To securely verify user identities via the national single sign-on platform.' }, { id: 'int4', name: 'Mali" Financial System', status: 'active', icon: <DollarSign/>, description: 'To manage and track grant payments and financial reporting.' } ];
export let mockNotifications = {
    applicant: [ { id: 1, icon: <CheckCircle className="text-green-500"/>, text: 'Congratulations! Your application "Our Beaches Without Plastic" has been approved.', time: '2 hours ago', unread: true }, { id: 2, icon: <Clock className="text-yellow-500"/>, text: 'Reminder: The Q2 report for grant APP-00124 is due in 1 week.', time: '1 day ago', unread: false}, { id: 5, icon: <XCircle className="text-red-500"/>, text: 'Unfortunately, your application "Reforestation Project in Asir Region" was rejected.', time: '3 days ago', unread: false} ],
    reviewer: [ { id: 3, icon: <FileText className="text-blue-500"/>, text: 'New application "AI-Powered Air Quality Monitoring" assigned for your review.', time: '4 hours ago', unread: true} ],
    admin: [ { id: 4, icon: <PlusCircle className="text-green-500"/>, text: '5 new applications were submitted yesterday.', time: '8 hours ago', unread: true} ],
    editor: []
};

export const sendGlobalAnnouncement = (title, message) => {
    const newAnnouncement = { id: Date.now(), icon: <Megaphone className="text-blue-500"/>, text: `${title}: ${message}`, time: 'Just now', unread: true };
    Object.keys(mockNotifications).forEach(role => {
        mockNotifications[role].unshift(newAnnouncement);
    });
};
