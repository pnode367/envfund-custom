...
import {mockUsers,mockIntegrations,mockDocumentTemplates,mockDataRoomFiles,sendGlobalAnnouncement } from '../utils/mockData';
...
export const EditorDashboard = ({ t }) => { return ( <div className="space-y-6"> <h2 className="text-2xl font-bold">{t('welcome')}, {mockUsers['editor@demo.com'].name}</h2> <div className="grid grid-cols-1 md:grid-cols-3 gap-6"> <StatCard title="Document Templates" value={mockDocumentTemplates.length} icon={<FileCheck2 />} color="blue" /> <StatCard title="Reviews Required" value="3" icon={<Edit />} color="yellow" /> <StatCard title="Outgoing Mail" value="25" icon={<Send />} color="green" /> </div> </div> ); };
export const DocumentTemplatesView = ({ t }) => { return ( <div className="bg-white p-6 rounded-2xl shadow-sm"> <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-bold">Manage Document Templates</h2><button className={`bg-${theme.primary} text-white font-bold py-2 px-4 rounded-lg flex items-center gap-2`}><PlusCircle size={18}/> Create New Template</button></div> <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{mockDocumentTemplates.map(t => ( <div key={t.id} className="border rounded-lg p-4 hover:shadow-md space-y-2 flex flex-col justify-between"><div><p className="font-bold text-lg">{t.name}</p><p className="text-sm text-gray-600">{t.description}</p></div><div className="flex justify-end gap-2 pt-2"><button className="text-xs bg-gray-100 rounded-md p-2 hover:bg-gray-200"><Edit size={14}/></button><button className="text-xs bg-gray-100 rounded-md p-2 hover:bg-gray-200"><Trash2 size={14}/></button></div></div>))}</div> </div> ); };
export const AssignReviewerModal = ({ application, onClose, t }) => { return ( <Modal onClose={onClose} title={`Assign Application ${application.id}`}> <div> <p className="mb-4">Select a reviewer for: <strong>{application.title}</strong></p> <select className="w-full mt-2 p-3 border rounded-lg bg-white">{Object.values(mockUsers).filter(u => u.role === 'reviewer').map(r => <option key={r.email} value={r.email}>{r.name}</option>)}</select> <div className="flex justify-end gap-4 pt-6"><button type="button" onClick={onClose} className="py-2 px-6 bg-gray-200 rounded-lg">{t('cancel')}</button><button type="submit" className={`py-2 px-6 bg-${theme.primary} text-white rounded-lg`}>Assign</button></div> </div> </Modal> ); };
export const AddUserModal = ({ onClose, t }) => { return ( <Modal onClose={onClose} title="Add New User"> <form className="space-y-4"> <div><label className="font-medium">Full Name</label><input type="text" className="w-full mt-2 p-3 border rounded-lg"/></div> <div><label className="font-medium">{t('email')}</label><input type="email" className="w-full mt-2 p-3 border rounded-lg"/></div> <div><label className="font-medium">Role</label><select className="w-full mt-2 p-3 border rounded-lg bg-white"><option value="applicant">{t('applicant')}</option><option value="reviewer">{t('reviewer')}</option><option value="editor">{t('editor')}</option><option value="admin">{t('admin')}</option></select></div> <div className="flex justify-end gap-4 pt-4"><button type="button" onClick={onClose} className="py-2 px-6 bg-gray-200 rounded-lg">{t('cancel')}</button><button type="submit" className={`py-2 px-6 bg-${theme.primary} text-white rounded-lg`}>Add User</button></div> </form> </Modal> ); };
